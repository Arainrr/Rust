...
{
    [Info("Discord Core", "MJSU", "0.16.10")]
    ...
    internal class DiscordCore : CovalencePlugin
    {
        ...
        private void FinishChunk()
        {
            ...
            Puts($"{_discordServer.members.Count} Total Discord Users Loaded");
        ...
        private void Discord_MessageCreate(Message message)
        {
            ...
            if (channel != null) 
            {
                ...
                if (!string.IsNullOrEmpty(_pluginConfig.DiscordJoinCommand) && content.ToLower().StartsWith("/" + _pluginConfig.DiscordJoinCommand))
                {
                    if (_useExternal)
                    {
