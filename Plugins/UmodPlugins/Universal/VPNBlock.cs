        ...
        void HandleIPHubResponse (IPlayer player, string url, string ip, int code, string response)
        {
            if (code != 200 || string.IsNullOrEmpty (response)) {
                if(debug)
                {
                    ...
                    PrintError ($"Service (IPHub) error: {message}");
                }
                else
                {
                    PrintError ("Service (IPHub) temporarily offline");
                }
            } else {
        ...
        void HandleIPStackResponse (IPlayer player, string url, string ip, int code, string response)
        {
            if (code != 200 || string.IsNullOrEmpty (response)) {
                if(debug)
                {
                    ...
                    PrintError ($"Service (IPStack) error: {message}");
                }
                else
                {
                    PrintError ("Service (IPStack) temporarily offline");
                }
            } else {
                JObject json;
                try {
