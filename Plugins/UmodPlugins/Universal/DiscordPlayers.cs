...
{
    [Info("Discord Players", "MJSU", "0.12.0")]
    ...
    internal class DiscordPlayers : CovalencePlugin
    {
        ...
        {
            SendMessage(channelId, GetPlayers(player));
            return null;
        }
        ...
        private string GetPlayers(IPlayer player)
        {
            string[] playerNames = players.Connected.Select(GetDisplayName).ToArray();

            string playersList = string.Join(_pluginConfig.Separator, playerNames);
            return Lang(LangKeys.OnlineFormat, player, playerNames.Length, playersList);
        }
