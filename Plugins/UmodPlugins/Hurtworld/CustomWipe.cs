...
namespace Oxide.Plugins
{
    ...
    class CustomWipe : HurtworldPlugin
    {
        ...
        private void ForcePlayerRemoval(PlayerSession session)
        {
            ...
            Singleton<HNetworkManager>.Instance.NetDestroy(session.WorldPlayerEntity.HNetworkView());
        ...
        void cmdcustomwipe(PlayerSession session, string command, string[] args)
        {
            ...
            {
                hurt.SendChatMessage(session, null, Msg("cmds", session.SteamId.ToString()));

                hurt.SendChatMessage(session, null, Msg("cmds_safecell", session.SteamId.ToString()));
                hurt.SendChatMessage(session, null, Msg("cmds_removecell", session.SteamId.ToString()));
                hurt.SendChatMessage(session, null, Msg("cmds_list", session.SteamId.ToString()));
                hurt.SendChatMessage(session, null, Msg("cmds_safehome", session.SteamId.ToString()));
                hurt.SendChatMessage(session, null, Msg("cmds_safeinv", session.SteamId.ToString()));
                hurt.SendChatMessage(session, null, Msg("cmds_tpcell", session.SteamId.ToString()));
                hurt.SendChatMessage(session, null, Msg("cmds_wipeinv", session.SteamId.ToString()));
                hurt.SendChatMessage(session, null, Msg("cmds_wipeobj", session.SteamId.ToString()));
            }
            else
        ...
        void cmdlistcells(PlayerSession session, string command, string[] args)
        {
            if (permission.UserHasPermission(session.SteamId.ToString(), "customwipe.use") || session.IsAdmin)
            {
                ...
                hurt.SendChatMessage(session, null, Msg("celllist", session.SteamId.ToString()));
                foreach (Cell cell in safeCells)
                {
                    hurt.SendChatMessage(session, null, Msg("celllist_format", steamid).Replace("{index}", index + "").Replace("{cellid}", cell.cellid + "").Replace("{description}", cell.description));
                    index++;
                }
                if (index == 0)
                    hurt.SendChatMessage(session, null, Msg("celllist_empty", session.SteamId.ToString()));
                else
                    hurt.SendChatMessage(session, null, Msg("tpcell", session.SteamId.ToString()));
            }
            else
                hurt.SendChatMessage(session, null, Msg("nopermission", session.SteamId.ToString()));
        }
        ...
        void cmdlistinv(PlayerSession session, string command, string[] args)
        {
            if (permission.UserHasPermission(session.SteamId.ToString(), "customwipe.use") || session.IsAdmin)
            {
                var steamid = session.SteamId.ToString();
                var index = 0;
                hurt.SendChatMessage(session, null, Msg("invlist", steamid));
                foreach (ulong playerid in safeInv)
                {
                    var playername = GameManager.Instance.GetIdentity(playerid)?.Name;
                    if (playername == null)
                        playername = "NoName";
                    hurt.SendChatMessage(session, null, Msg("invlist_format", steamid).Replace("{index}", index + "").Replace("{playerid}", playerid + "").Replace("{playerName}", playername));
                    index++;
                }
                if (index == 0)
                    hurt.SendChatMessage(session, null, Msg("invlist_empty", session.SteamId.ToString()));
            }
            else
                hurt.SendChatMessage(session, null, Msg("nopermission", session.SteamId.ToString()));
        }
        ...
        void cmdremovecell(PlayerSession session, string command, string[] args)
        {
            if (permission.UserHasPermission(session.SteamId.ToString(), "customwipe.use") || session.IsAdmin)
            {
                ...
                if (safeCells.Any(c => c.cellid == cell))
                {
                    ...
                    SaveData();
                    hurt.SendChatMessage(session, null, Msg("removecell", session.SteamId.ToString()).Replace("{cell}", cell + ""));
                }
                else
                    hurt.SendChatMessage(session, null, Msg("removecell_removed", session.SteamId.ToString()).Replace("{cell}", cell + ""));
            }
            else
                hurt.SendChatMessage(session, null, Msg("nopermission", session.SteamId.ToString()));
        }
                    ...
                    foreach (PlayerIdentity identity in GameManager.Instance.GetIdentifierMap().Values)
                    {
                            ...
                            identity.SteamId.ToString().Equals(args[0]))
                        {
                            ...
                            if (!safeInv.Contains(playerid))
                            {
                                ...
                                hurt.SendChatMessage(session, null, Msg("safeinv_safe", session.SteamId.ToString()).Replace("{playername}", identity.Name));
                            }
                            else
                            {
                                safeInv.Remove(playerid);
                                hurt.SendChatMessage(session, null, Msg("safeinv_remove", session.SteamId.ToString()).Replace("{playername}", identity.Name));
                            }
                            SaveData();
                            return;
                        }
                ...
                if (args.Length == 1)
                {
                    ...
                    foreach (PlayerIdentity identity in GameManager.Instance.GetIdentifierMap().Values)
                    {
                            ...
                            identity.SteamId.ToString().Equals(args[0]))
                        {
                            ...
                            SaveData();
                            hurt.SendChatMessage(session, null, Msg("safehome_safe", session.SteamId.ToString()).Replace("{playername}", identity.Name).Replace("{count}", count + ""));
                            return;
                        }
                    }
                    if (!found)
                        hurt.SendChatMessage(session, null, Msg("safehome_notFound", session.SteamId.ToString()).Replace("{playername}", args[0]));
                }
            }
            else
                hurt.SendChatMessage(session, null, Msg("nopermission", session.SteamId.ToString()));
        ...
        void cmdsafecell(PlayerSession session, string command, string[] args)
        {
            if (permission.UserHasPermission(session.SteamId.ToString(), "customwipe.use") || session.IsAdmin)
            {
                ...
                if (!safeCells.Any(c => c.cellid == cell))
                {
                    ...
                    SaveData();
                    hurt.SendChatMessage(session, null, Msg("safecell", session.SteamId.ToString()).Replace("{cell}", cell + ""));
                }
                else
                    hurt.SendChatMessage(session, null, Msg("safecell_safe", session.SteamId.ToString()).Replace("{cell}", cell + ""));
            }
            else
                hurt.SendChatMessage(session, null, Msg("nopermission", session.SteamId.ToString()));
        }
        ...
        void cmdwipe(PlayerSession session, string command, string[] args)
        {
            if (permission.UserHasPermission(session.SteamId.ToString(), "customwipe.use") || session.IsAdmin)
            {
                ...
                hurt.SendChatMessage(session, null, Msg("destroyed", session.SteamId.ToString()).Replace("{Count}", count.ToString()));
