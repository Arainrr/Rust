...
{
    [Info("HW Custom Map Name", "klauz24", "1.5.1"), Description("Changes map name at the server list")]
    internal class HWCustomMapName : HurtworldPlugin
    {
        ...
        protected override void LoadDefaultMessages()
        {
            ...
            {
                {"HWCustomMapName - Days", "{0}d. {1}h."},
                {"HWCustomMapName - Hours", "{0}h. {1}m."},
                {"HWCustomMapName - Minutes", "{0}m. {1}s."},
                {"HWCustomMapName - Seconds", "{0}s."}
            }, this);
        }
        ...
        private string FormatTime(double time)
        {
            ...
            {
                return string.Format(Lang("HWCustomMapName - Days"), timeSpan.Days, timeSpan.Hours);
            }
            if (Math.Floor(timeSpan.TotalMinutes) >= 60)
            {
                return string.Format(Lang("HWCustomMapName - Hours"), timeSpan.Hours, timeSpan.Minutes);
            }
            if (Math.Floor(timeSpan.TotalSeconds) >= 60)
            {
                return string.Format(Lang("HWCustomMapName - Minutes"), timeSpan.Minutes, timeSpan.Seconds);
            }
            return string.Format(Lang("HWCustomMapName - Seconds"), timeSpan.Seconds);
        }

        private string Lang(string key) => lang.GetMessage(key, this);

        private void UpdateMapName(string str) => SteamGameServer.SetMapName(str);
    }
}