    ...
    class KillCounter : HurtworldPlugin
    {
        ...
        private void OnPlayerDeath(PlayerSession playerSession, EntityEffectSourceData dataSource)
        {
            if (CustomDeathMessages == null)
            {
                ...
                string killerkills = AddKill(playerSession, dataSource);
                if (killerkills != null)
                {
                    timer.Once(1f, () =>
                    {
                        Server.Broadcast(lang.GetMessage("player", this).Replace("{Name}", killer.Identity.Name).Replace("{Kills}", killerkills));
                    });
                }
            }
        }
