...
namespace Oxide.Plugins
{
    ...
    [Info("Wager of battle", "Pho3niX90", "0.1.4")]
    class Duel : HurtworldPlugin
    {
        ...
        void OnPlayerDeath(PlayerSession player, EntityEffectSourceData source) {
            ...
            if (isPlayer) {
                ...
                //    getSession(thisDuel.Requester.ToString()).WorldPlayerEntity.GetComponent<EntityStats>().GetFluidEffect(EntityFluidEffectKeyDatabase.Instance.).SetValue(thisDuel.ReqInfamy);
                //    getSession(thisDuel.Accepter.ToString()).WorldPlayerEntity.GetComponent<EntityStats>().GetFluidEffect(EntityFluidEffectKeyDatabase.Instance.).SetValue(thisDuel.AccInfamy);
                //}
                //
            ...
            switch (Action) {
                ...
                case "req":
                    ...
                    PrintToChat(player, GetMsg("msg_DuellRequestSent", player)
                                                .Replace("{playerName}", accepter.Identity.Name));

                    if (requestorBalance >= pot) {
                        pot *= 2;
                        string thepos = player.WorldPlayerEntity.transform.position.ToString();
                        Duels.Add(new DuelData((ulong)player.SteamId, player.Identity.Name, (ulong)accepter.SteamId, accepter.Identity.Name, pot, thepos));
        ...
        void doDuel(DuelData Duel) {
            ...
            timer.Once(float.Parse(GetConf("transportTime", "60").ToString()), () => {
                playerEntity1 = player1.WorldPlayerEntity.gameObject;
                playerEntity2 = player2.WorldPlayerEntity.gameObject;

                hurt.BroadcastChat(GetMsg("broadcast_DuelStarting")
                                    .Replace("{player1}", getSession(Duel.Accepter.ToString()).Identity.Name)
                                    .Replace("{player2}", getSession(Duel.Requester.ToString()).Identity.Name));
                timer.Once(1f, () => {
        ...
        void doEmote(PlayerSession player) {
            ...
            //EmoteServer.BeginEmoteServer(EmoteT);
        }
        ...
        bool isEcoLoaded() {
            if (EconomyBanks != null) { return true; } else { return false; }
        }
        #endregion
    }
}