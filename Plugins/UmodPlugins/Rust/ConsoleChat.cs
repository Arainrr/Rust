    ...
    class ConsoleChat : RustPlugin
    {
        ...
        void OnPlayerChat(BasePlayer player, string message, Chat.ChatChannel channel)
        {
            if (channel == Chat.ChatChannel.Team)
            {
                ...
                {
                    if (!RelationshipManager.Instance.cachedPlayers.TryGetValue(teamMate, out cachedTeamPlayer) || permission.UserHasPermission(cachedTeamPlayer.UserIDString, perm))
                    {
                        continue;
                    }
