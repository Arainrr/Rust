...
namespace Oxide.Plugins
{
    ...
    public class PropControl : RustPlugin
    {
        ...
        private void Init()
        {
            ...
            PrintWarning("Plugin initialized");
        }
        ...
        private void LeaveCommand(BasePlayer player)
        {
            ...
            if (index == -1)
            {
                PrintToChat(player, Message("PlayerNotAProp", player.UserIDString)); // api
                return;
            }
            ...
            PrintToChat(player, Message("PlayerLeftAProp", player.UserIDString)); // api
        }
        ...
        private void BecomeCommand(BasePlayer player, string command, string[] args)
        {
            ...
            {
                PrintToChat(player, Message("NoPermissions", player.UserIDString)); // api
                return;
            }
            ...
            {
                PrintToChat(player, Message("NoVanshPermission", player.UserIDString)); // api
                return;
            }
            if ((Vanish == null || !Vanish.IsLoaded || !_pluginConfig.PropControlConfig.UseVanish)
                && (BetterVanish == null || !BetterVanish.IsLoaded || !_pluginConfig.PropControlConfig.UseBetterVanish)) // null check
            {
                PrintToChat(player, Message("NoVanishPlugin", player.UserIDString)); // api
                return;
            }
            if (args.Length < 1)
            {
                PrintToChat(player, Message("InvalidArgs", player.UserIDString)); // api
                return;
            }
            int index = data.FindIndex(data => data.player == player);
            if (index != -1) // removed linq
            {
                ...
                {
                    PrintToChat(player, Message("PlayerAlreadyAProp", player.UserIDString)); // api
                    return;
                }
                else
                {
            ...
            if (prop == null)
            {
                PrintToChat(player, Message("UnknownError", player.UserIDString)); // api
                return;
            }
            Apex.AI.Components.UtilityAIComponent utilityComponent;
            if (prop.TryGetComponent(out utilityComponent))
            {
                UnityEngine.Object.Destroy(utilityComponent);
            }
            ...
            PlayerInput playerInout;
            if (prop.TryGetComponent(out playerInout))
            {
                UnityEngine.Object.Destroy(playerInout);
            }
            ...
            PrintToChat(player, Message("PlayerBecameAProp", player.UserIDString, args[0])); // api
        }

        [ChatCommand("props")]
        private void PropsCommand(BasePlayer player)
        {
            PrintToChat(player, Message("PropsList", player.UserIDString, string.Join("\n<color=#FFA500>•</color> ", _pluginConfig.PropControlConfig.PropPrefabMap.Keys))); // api
        }
