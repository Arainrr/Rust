...
{
    [Info("Sky Turrets", "Xavier", "3.0.5")]
	[Description("Allows auto turrets and SamSites to target Patrol Helicopters and Chinooks")]
	...
	public class SkyTurrets : RustPlugin
    {
	   ...
	   #region Localization
		...
		private class AntiHeli : MonoBehaviour
		{
			...
			{
				if (target != null  && turret.IsOnline()) // Check if there is a target and the turret is Online
				{
            ...
            {
				if (col.name == "patrol_helicopter" || col.name == "ch47_collider_front_motor") // Forcing to check if online 
				{
