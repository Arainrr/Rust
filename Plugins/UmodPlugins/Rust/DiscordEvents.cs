...
{
    [Info("Discord Events", "MON@H", "1.1.0")]
    ...
    class DiscordEvents : CovalencePlugin
    {
        ...
        private void OnServerShutdown()
        {
            if (_configData.ServerStateSettings.Enabled)
            {
                ...
                SendMessage(Lang("Shutdown"), _configData.ServerStateSettings.WebhookURL);
            }
        }
        ...
        private void OnEntityDeath(BasePlayer player, HitInfo info)
        {
            ...
            if (_configData.PlayerDeathSettings.Enabled)
            {
                ...
                SendMessage(Lang("Death", null, player.displayName), _configData.PlayerDeathSettings.WebhookURL);
            }
        }
        ...
        private void OnPlayerConnected(BasePlayer player)
        {
            if (_configData.PlayerConnectedSettings.Enabled)
            {
                ...
                SendMessage(Lang("PlayerConnected", null, player.displayName), _configData.PlayerConnectedSettings.WebhookURL);
            }

            if (_configData.PlayerConnectedInfoSettings.Enabled)
            {
                SendMessage(Lang("PlayerConnectedInfo", null, player.displayName, player.UserIDString, player.net.connection.ipaddress.Split(':')[0]), _configData.PlayerConnectedInfoSettings.WebhookURL);
            }
        }
        ...
        private void OnPlayerDisconnected(BasePlayer player, string reason)
        {
            ...
            if (_configData.PlayerDisconnectedSettings.Enabled)
            {
                ...
                SendMessage(Lang("PlayerDisconnected", null, player.displayName, reason), _configData.PlayerConnectedSettings.WebhookURL);
            }
        }

        private void OnPlayerChat(BasePlayer player, string message, ConVar.Chat.ChatChannel channel)
        {
            if (player == null || string.IsNullOrEmpty(message) || string.IsNullOrWhiteSpace(message))
            {
                return;
            }
            ...
            {
                SendMessage(Lang("Chat", null, player.displayName, message), _configData.ChatSettings.WebhookURL);
            }

            if (channel == ConVar.Chat.ChatChannel.Team && _configData.ChatTeamSettings.Enabled)
            {
                SendMessage(Lang("ChatTeam", null, player.displayName, message), _configData.ChatTeamSettings.WebhookURL);
            }
        }
            ...
            if (_configData.SupplyDropSettings.Enabled)
            {
                NextTick(() =>
                {
                    if (player != null && entity != null)
                    {
                        ...
                        SendMessage(Lang("SupplySignal", null, player.displayName, GetGridPosition(entity.transform.position)), _configData.SupplyDropSettings.WebhookURL);
                    }
                });
            }
        }
