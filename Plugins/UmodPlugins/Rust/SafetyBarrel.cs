...
{
    [Info("Safety Barrel", "Dana", "0.0.8")]
    [Description("Hides players from scientists and Bradley while wearing the barrel costume.")]
                ...
                if (permission.UserHasPermission(player.UserIDString, SAFETYBARREL_PROTECT))
                {
                    ...
                    foreach (Item item in playerInv_Clothing.itemList)
                    {
                        if (item != null)
                        {
                            ...
                            if (itemDef.shortname == "barrelcostume" || itemDef.shortname == "cratecostume")
                            {
                                return true;
                            }
                        }
