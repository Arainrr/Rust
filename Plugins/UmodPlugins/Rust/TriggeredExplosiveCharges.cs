...
{
    [Info("Triggered Explosive Charges", "EnigmaticDragon", "1.0.23")]
    ...
    public class TriggeredExplosiveCharges : RustPlugin
    {
        ...
        private class TriggeredExplosivesManager
        {
            ...
            public TriggeredExplosivesManager(BasePlayer player)
            {
                ...
                if (!saveData.deployedExplosives.TryGetValue(player.userID, out deployedExplosives))
                ...
                else
                {
                    for (int i = deployedExplosives.Count - 1; i > -1; i--)
                    {
                        ...
                        {
                            DroppedItem fakeC4;
                            TimedExplosive realC4;
                            if (fakeC4 = bn.GetComponent<DroppedItem>())
                            {
                                fakeC4_To_PlayerID[fakeC4.net.ID] = player.userID;
                                fakeC4.allowPickup = config.IsPickupAllowed;
                                fakeExplosives.Add(fakeC4);
                            }
                            else if (realC4 = bn.GetComponent<TimedExplosive>())
                            {
            ...
            {
                if (c4_mode == (ulong)C4_MODE.TIMED || (config.IsBlockedInEvent && Instance.IsPlayingEvent(player)))
                    return;

                if (config.AreDeployableChargesLimited && config.MaxDeployableCharges <= (triggeredExplosives.Count + fakeExplosives.Count))
                {
                    ChatMessage(player, L_LIMIT_REACHED, config.MaxDeployableCharges);
                    return;
                }
                ...
                if (config.ShouldDisableBeeping)
                {
                    Instance.timer.Once(config.BeepDuration, () =>
                    {
                        if (te != null)
                            RealC4_To_FakeC4(te);
                    });
            ...
            public void Explode(bool forceExplode = false)
            {
                ...
                if (config.IsTriggerOnTimeUse && triggeredExplosives.Count > 0)
                    trigger.Remove();
            ...
            private void RealC4_To_FakeC4(TimedExplosive realC4)
            {
                ...
                worldModel.allowPickup = config.IsPickupAllowed;
            ...
            public void CraftTrigger()
            {
                ...
                {
                    item_1_amount += container.GetAmount(config.IngredientId1, true);
                    item_2_amount += container.GetAmount(config.IngredientId2, true);
                }

                if (item_1_amount >= config.IngredientAmount1 && item_2_amount >= config.IngredientAmount2)
                {
                    item_1_amount = config.IngredientAmount1;
                    item_2_amount = config.IngredientAmount2;

                    foreach (ItemContainer container in containers)
                    {
                        if (item_1_amount > 0)
                            item_1_amount -= container.Take(null, config.IngredientId1, item_1_amount);

                        if (item_2_amount > 0)
                            item_2_amount -= container.Take(null, config.IngredientId2, item_2_amount);
                    }
                    ...
                    if (GiveTrigger(1) != 1)
                    {
                        ...
                        player.inventory.GiveItem(ItemManager.Create(ItemManager.FindItemDefinition(config.IngredientId1), config.IngredientAmount1));
                        player.inventory.GiveItem(ItemManager.Create(ItemManager.FindItemDefinition(config.IngredientId2), config.IngredientAmount2));
                        return;
                    }
                    ...
                    ChatMessage(player, L_CRAFTING_FAILED_RESOURCES, new object[] {
                        ItemManager.FindItemDefinition(config.IngredientId1).displayName.translated + " (" +
                        config.IngredientAmount1 + "), " + ItemManager.FindItemDefinition(config.IngredientId2).displayName.translated
                        + " (" + config.IngredientAmount2 + ")" });
            }
        ...
        private static class TriggerShop
        {
            ...
            {
                foreach (Item i in vendingMachine.inventory.FindItemsByItemID(config.CurrencyId))
            ...
            public static void SetupSellOrder(VendingMachine vendingMachine)
            {
                ...
                order.currencyID = config.CurrencyId;
                order.currencyAmountPerItem = config.CurrencyNeeded;
        ...
        {
            if (!config.IsCraftingEnabled && (player.net.connection.authLevel < 2) && !PermissionGranted(player, PERMISSION_CRAFTING))
            {
                ChatMessage(player, L_PERMISSION_FAILED);
                return;
            }
        ...
        {
            if (config.IsPickupAllowed && item.info.itemid == C4_ITEM_ID)
        ...
        {
            private const string DefaultIngredient1 = "metal.refined";
            private const string DefaultIngredient2 = "techparts";
            private const string DefaultCurrency = "techparts";

            [JsonProperty(PropertyName = "SHOP | Currency item for buying a trigger [item shortname]")]
            public string CurrencyShortname { get; private set; } = DefaultCurrency;
            [JsonProperty(PropertyName = "SHOP | Needed amount of currency item [number]")]
            public int CurrencyNeeded { get; private set; } = 5;

            [JsonProperty(PropertyName = "CRAFTING | Allow all players to craft triggers [true,false]")]
            public bool IsCraftingEnabled { get; private set; } = false;

            [JsonProperty(PropertyName = "CRAFTING | Ingredient 1 for crafting [item shortname]")]
            public string IngredientShortname1 { get; private set; } = DefaultIngredient1;
            [JsonProperty(PropertyName = "CRAFTING | Needed amount of ingredient 1 [number]")]
            public int IngredientAmount1 { get; private set; } = 50;

            [JsonProperty(PropertyName = "CRAFTING | Ingredient 2 for crafting [item shortname]")]
            public string IngredientShortname2 { get; private set; } = DefaultIngredient2;
            [JsonProperty(PropertyName = "CRAFTING | Needed amount of ingredient 2 [number]")]
            public int IngredientAmount2 { get; private set; } = 2;

            [JsonProperty(PropertyName = "TRIGGERED C4 | Disable beeping sound after duration (minimum: 5; infinite: -1) [seconds]")]
            public int BeepDuration { get; private set; } = 10;
            [JsonProperty(PropertyName = "TRIGGERED C4 | Allow C4 pickup (only after beeping got disabled) [true, false]")]
            public bool IsPickupAllowed { get; private set; } = false;
            [JsonProperty(PropertyName = "TRIGGERED C4 | Limit for number of C4 that can be deployed in triggered mode (minimum: 1; infinite: -1) [number]")]
            public int MaxDeployableCharges { get; private set; } = -1;

            [JsonProperty(PropertyName = "TRIGGER | Trigger destroys itself after using it once [true, false]")]
            public bool IsTriggerOnTimeUse { get; private set; } = false;

            [JsonProperty(PropertyName = "Event Manager | Block trigger usage during events [true, false]")]
            public bool IsBlockedInEvent { get; private set; } = false;

            [JsonIgnore] public int CurrencyId { get; private set; } = ItemManager.FindItemDefinition(DefaultCurrency).itemid;
            [JsonIgnore] public int IngredientId1 { get; private set; } = ItemManager.FindItemDefinition(DefaultIngredient1).itemid;
            [JsonIgnore] public int IngredientId2 { get; private set; } = ItemManager.FindItemDefinition(DefaultIngredient2).itemid;

            [JsonIgnore] public bool ShouldDisableBeeping { get { return BeepDuration != -1; } }
            [JsonIgnore] public bool AreDeployableChargesLimited { get { return MaxDeployableCharges != -1; } }

            [OnDeserialized]
            internal void OnDeserializedMethod(StreamingContext context)
            {
                ItemDefinition ingredientDef1 = ItemManager.FindItemDefinition(IngredientShortname1);
                ItemDefinition ingredientDef2 = ItemManager.FindItemDefinition(IngredientShortname2);
                ItemDefinition currencyDef = ItemManager.FindItemDefinition(CurrencyShortname);

                if (ingredientDef1 != null)
                {
                    IngredientId1 = ingredientDef1.itemid;
                }
                else
                {
                    ServerMessage(L_CONFIG_ERROR, new object[] { "Ingredient 1 for crafting", DefaultIngredient1 });
                    IngredientShortname1 = DefaultIngredient1;
                }
                if (ingredientDef2 != null)
                {
                    IngredientId2 = ingredientDef2.itemid;
                }
                else
                {
                    ServerMessage(L_CONFIG_ERROR, new object[] { "Ingredient 2 for crafting", DefaultIngredient2 });
                    IngredientShortname2 = DefaultIngredient2;
                }
                if (currencyDef != null) CurrencyId = currencyDef.itemid;
                else
                {
                    ServerMessage(L_CONFIG_ERROR, new object[] { "Currency item for buying a trigger", DefaultCurrency });
                    CurrencyShortname = DefaultCurrency;
                }

                if (BeepDuration != -1) BeepDuration = Mathf.Max(BeepDuration, 5);
                if (MaxDeployableCharges != -1) MaxDeployableCharges = Mathf.Max(MaxDeployableCharges, 1);
            }
        }
        #endregion
        ...
        private const string L_LIMIT_REACHED = "Triggered C4 | Limit";
        ...
        private static void ChatMessage(BasePlayer player, string key, params object[] args) { player.ChatMessage(String.Format(Instance.lang.GetMessage(key, Instance, player.UserIDString), args)); }
        private static void ServerMessage(string key) { Instance.Puts(Instance.lang.GetMessage(key, Instance)); }
        private static void ServerMessage(string key, params object[] args) { Instance.Puts(String.Format(Instance.lang.GetMessage(key, Instance), args)); }
        ...
        private new void LoadDefaultMessages()
        {
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                [L_LIMIT_REACHED] = "You can not deploy more than {0} charges in triggered mode"
            }, this);
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ...
                [L_LIMIT_REACHED] = "Du kannst nicht mehr als {0} Sprengladungen im Trigger Modus verwenden"
            }, this, "de");
        }
