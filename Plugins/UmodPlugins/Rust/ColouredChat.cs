...
namespace Oxide.Plugins
{
    ...
    class ColouredChat : CovalencePlugin
    {
        ...
        private object OnPlayerChat(BasePlayer player, string message, Chat.ChatChannel channel)
        {
            ...
            var colouredChatMessage = FromMessage(player.IPlayer, channel, message, player.displayName);
        ...
        private void OnBetterChat(Dictionary<string, object> dict)
        {
            if (dict != null)
            {
                var player = dict["Player"] as IPlayer;
                var username = dict["Username"].ToString();
                var colouredChatMessage = FromMessage(player, (Chat.ChatChannel)dict["ChatChannel"], dict["Message"].ToString(), username);
                var colouredChatMessageDict = colouredChatMessage.ToDictionary();
                colouredChatMessage = ColouredChatMessage.FromDictionary(colouredChatMessageDict);

                if (!string.IsNullOrEmpty(colouredChatMessage.Name))
                {
                    dict["Username"] = colouredChatMessage.Name;
                }
                
                if (!string.IsNullOrEmpty(colouredChatMessage.Colour))
                {
        ...
        private ColouredChatMessage FromMessage(IPlayer player, Chat.ChatChannel channel, string message, string username)
        {
            var playerData = new PlayerData();
            if (allColourData.ContainsKey(player.Id)) playerData = allColourData[player.Id];
            var colouredNameData = GetColouredName(player, playerData, username);
        ...
        private ColouredChatMessage GetColouredName(IPlayer player, PlayerData playerData, string username)
        {
            ...
            if (!cachedData.ContainsKey(player.Id))
            {
                ...
                if (playerData?.NameGradientArgs != null) 
                    gradientName = ProcessGradient(playerUserName, playerData.NameGradientArgs, false, player);
            ...
            if (HasNameShowPerm(player))
            {
                ...
                else if (playerUserName == username && !string.IsNullOrEmpty(cachedData[player.Id].NameColourGradient)) playerUserName = cachedData[player.Id].NameColourGradient;
            }
            ...
            if (allColourData.ContainsKey(userPrimaryGroup))
            {
                ...
                if (playerUserName == username && playerColour == playerColourNonModified)
                {
                    if (groupData?.NameGradientArgs != null) playerUserName = string.IsNullOrEmpty(cachedData[player.Id].NameColourGradient) ?
                            cachedData[player.Id].NameColourGradient = ProcessGradient(username, groupData.NameGradientArgs, false, player) :
        ...
        private string API_GetColouredName(IPlayer player)
        {
            ...
            var colouredData = GetColouredName(player, playerData, player.Name);
        ...
        private string API_GetColouredChatMessage(IPlayer iPlayer, Chat.ChatChannel channel,
            string message)
        {
            var colouredChatMessage = FromMessage(iPlayer, channel, message, iPlayer.Name);
