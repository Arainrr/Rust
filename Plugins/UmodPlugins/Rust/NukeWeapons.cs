...
{
    [Info("NukeWeapons", "k1lly0u", "0.1.67")]
    ...
    class NukeWeapons : RustPlugin
    {
        ...
        private void OnServerInitialized()
        {
            ...
            {
                foreach (KeyValuePair<string, ItemDefinition> item in itemDefs)
                {
        ...
        private void Unload()
        {
            ...
            for (int i = 0; i < nwTimers.Count; i++)            
                nwTimers[i].Destroy();            
        ...
        private void OnEntitySpawned(BaseNetworkable entity)
        {
            ...
            {
                Landmine mine = entity as Landmine;
                if (activeUsers.ContainsKey(mine.OwnerID) && activeUsers[mine.OwnerID] == NukeType.Mine)
                {
                    BasePlayer player = BasePlayer.FindByID(mine.OwnerID);
                    if (player != null)
                    {
        ...
        private void OnPlayerAttack(BasePlayer attacker, HitInfo info)
        {
            if (activeUsers.ContainsKey(attacker.userID) && activeUsers[attacker.userID] == NukeType.Bullet)
            {
                ...
                {
                    string ammo = info?.Weapon?.GetEntity()?.GetComponent<BaseProjectile>()?.primaryMagazine?.ammoType?.shortname;
        ...
        {
            Dictionary<uint, Landmine> mineList = new Dictionary<uint, Landmine>();
            ...
            for (int i = 0; i < allobjects.Length; i++)
            {
                ...
                Landmine landmine = gobject.GetComponent<Landmine>();
                if (landmine != null)
                {
                    if (!mineList.ContainsKey(landmine.net.ID))
                        mineList.Add(landmine.net.ID, landmine);
                }
            }

            for (int i = 0; i < nukeData.Mines.Count; i++)
            {
                uint entry = nukeData.Mines[i];
        ...
        private bool FinishedCrafting(BasePlayer player)
        {
            ...
            foreach (KeyValuePair<NukeType, double> craft in craftingTimers[player.userID])
            {
                if (craft.Value != -1)
                {
                    finished = false;
        ...
        {            
            CuiElementContainer container = NWUI.CreateElementContainer(UIPanel, UIColors["dark"], "0 0", "1 0.92");

            NWUI.CreatePanel(ref container, UIPanel, UIColors["light"], "0.01 0.02", "0.99 0.98", true);
            ...
            foreach(KeyValuePair<string, int> item in ingredients)
            {
                ...
                CreateIngredientEntry(ref container, UIPanel, displayNames[itemInfo.shortname], item.Value, plyrAmount, i);
                i++;
            }
            ...
            else NWUI.CreateLabel(ref container, UIPanel, "", $"{cachedAmmo[player.userID][type]} / {config.MaxAllowed}", 16, "0.6 0.75", "0.9 0.85");

            NWUI.CreateButton(ref container, UIPanel, UIColors["buttonbg"], text, 16, $"0.6 0.65", $"0.74 0.72", command);

            if (cachedAmmo[player.userID][type] > 0 || HasUnlimitedAmmo(player))
            {
                if (IsSelectedType(player, type))
                    NWUI.CreateButton(ref container, UIPanel, UIColors["buttonbg"], MSG("Disarm", player.UserIDString), 16, $"0.76 0.65", $"0.9 0.72", $"NWUI_DeactivateMenu {type.ToString()}");
                else NWUI.CreateButton(ref container, UIPanel, UIColors["buttonbg"], MSG("Arm", player.UserIDString), 16, $"0.76 0.65", $"0.9 0.72", $"NWUI_Activate {type.ToString()}");
            }
        ...
        {               
            ...
            foreach(KeyValuePair<NukeType, double> craft in craftingTimers[player.userID])
            {
                if (craft.Value == -1)
                    ...
                    message += $"{craft.Key.ToString()}: {configData.Options.MSG_MainColor}{CraftTimeClock(player, craft.Key)}</color>     ";                              
            }
            ...
            else message = $"{configData.Options.MSG_MainColor}{MSG("Crafting", player.UserIDString)} ::: </color> " + message;
        ...
        private void AddButtons(BasePlayer player, int number)
        {
            ...
            NWUI.CreateButton(ref container, UIIcon, UIColors["buttonbg"], MSG("Menu",player.UserIDString), 16, "0 0.55", "1 1", "NWUI_OpenMenu");
            NWUI.CreateButton(ref container, UIIcon, UIColors["buttonbg"], MSG("Deactivate", player.UserIDString), 16, "0 0", "1 0.45", "NWUI_DeactivateIcons");
        ...
        private void cmdNWActivate(ConsoleSystem.Arg arg)
        {
            ...
            if (!activeUsers.ContainsKey(player.userID))
                activeUsers.Add(player.userID, nukeType);
            else activeUsers[player.userID] = nukeType;
