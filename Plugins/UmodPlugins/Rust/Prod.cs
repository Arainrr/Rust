...
namespace Oxide.Plugins
{
    ...
    class Prod : RustPlugin
    {
        ...
        void OnEntityBuilt(HeldEntity heldentity, GameObject gameobject)
        {
            ...
            IPlayer p = covalence.Players.FindPlayerByObj(player);
            SetBlockData(block, p.Id);
        }
        #endregion
        ...
        void cmdChatProd(BasePlayer player, string command, string[] args)
        {
            if (!hasAccess(player))
            {
                if (passiveMode)
                    ...
                    {
                        var input1 = serverinput.GetValue(player) as InputState;
                        var currentRot1 = Quaternion.Euler(input1.current.aimAngles) * Vector3.forward;
                        var target1 = DoRay(player.transform.position + eyesAdjust, currentRot1);
                        if (target1 == null)
                        {
                            SendReply(player, noTargetfound);
                            return;
                        }
                        Passive_GetCodelockWhitelisted(player, target1);
                        return;
                    }
                    else
                    {
