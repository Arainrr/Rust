...
{
    [Info("Dynamic Config", "FastBurst", "1.0.3")]
    ...
    class DynamicConfig : RustPlugin
    {
        ...
        private void OnServerInitialized()
        {
            ...
            this.WipeDate = this.WipeDate.ToLocalTime();
            Puts(_("WipeDate", this.WipeDate));
            Puts(_("TimePassed", TimeSpanToString(DateTime.Now.Subtract(this.WipeDate))));
            this.configLibrary = new ConfigLibrary();
            timer.Once(5, this.configLibrary.Update);
            this.updateTimer = timer.Every(60, this.configLibrary.Update);
        }

        private void Unload()
        {
            if (this.updateTimer != null && !this.updateTimer.Destroyed)
            {
        ...
        class ConfigLibrary
        {
            Dictionary<string, PluginInfo> pluginInfos = new Dictionary<string, PluginInfo>();
        ...
        class PluginInfo
        {
            ...
            {
                this.configs.Add(new ConfigInfo(filename, applyTime));
                this.configs.Sort((a, b) => a.applyTime.CompareTo(b.applyTime));
            }
