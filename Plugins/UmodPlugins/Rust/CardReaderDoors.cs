...
{
    [Info("Card Reader Doors", "Ts3Hosting", "2.0.15")]
    ...
    public class CardReaderDoors : RustPlugin
    {
        ...
        void OnEntityBuilt(Planner plan, GameObject go)
        {
            ...
            if (!permission.UserHasPermission(player.UserIDString, theadmin) || !activate.ContainsKey(player.userID))
            {
                return;
            }
            ...
            if (entity is BaseEntity)
            {
                if (entity.ShortPrefabName == "laserdetector")
                {
					...
					if (changed != null)
					changed.accessLevel = activate[player.userID];
                    reader.Spawn();
        ...
        object OnCardSwipe(CardReader reader1, Keycard card1, BasePlayer player)
        {
            ...
            var check = checkdoor(reader1.net.ID);
            ...
            Door door = BaseNetworkable.serverEntities.Find(Convert.ToUInt32(doorID)) as Door;
            if (door == null) return null;

            if (!door.IsOpen() && (door.GetComponent<Door>()))
            {
                door.SetFlag(BaseEntity.Flags.Open, true);				
                timer.Once(close, () => { door.SetFlag(BaseEntity.Flags.Open, false); });
				if (damage)
				{
        ...
        private void lockdoor(BasePlayer player, string command, string[] args)
        {
            ...
            if (args[0].ToLower() == "active")
            {
                ...
                if (activate.ContainsKey(player.userID))
                {
					activate.Remove(player.userID);
                ...
                else
                {
                    ...
                    activate.Add(player.userID, ids);
            ...
            if (rayResult is Door)
            {
                ...
                if (!pcdData.pEntity.ContainsKey(args[0].ToLower()))
                {
                    ...
                    pcdData.pEntity[args[0].ToLower()].doorID = entity.net.ID;
        ...
        private void setlock(BasePlayer player, string command, string[] args)
        {
            ...
            if (rayResult is CardReader)
            {
                ...
                pcdData.pEntity[args[0].ToLower()].reader.Add(entity.net.ID, new Reader());
                SaveData();
        ...
        {
            RaycastHit doorhit;
            if (!Physics.Raycast(player.eyes.HeadRay(), out doorhit)) {return null;}
