...
{
    [Info("ArenaWallGenerator", "nivex", "1.0.4")]
    ...
    public class ArenaWallGenerator : RustPlugin
    {
        ...
        public bool ZoneWallsExist(ulong ownerId, List<BaseEntity> entities)
        {
            if (entities == null || entities.Count < 3)
                entities = BaseNetworkable.serverEntities.Where(e => e?.name != null && e.name.Contains("wall.external.high")).OfType<BaseEntity>().ToList();
        ...
        public int RemoveZoneWalls(ulong ownerId)
        {
            ...
            foreach (var entity in BaseNetworkable.serverEntities.Where(e => e?.name != null && e.name.Contains("wall.external.high")).OfType<BaseEntity>().ToList())
            {
