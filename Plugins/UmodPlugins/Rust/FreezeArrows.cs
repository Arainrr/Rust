...
{
    [Info("FreezeArrows", "Colon Blow", "1.1.1")]
    class FreezeArrows : RustPlugin
    {
        // added config option to keep freeze arrow reloaded after shooting one
        // fixed reply to player when toggling off freeze arrows.

        #region Loadup

        List<uint> FrozenEntityList = new List<uint>();
        ...
        void Unload()
        {
            foreach (var player in BasePlayer.activePlayerList)
            {
                FrozenEntityList.Remove(player.net.ID);
                string guiInfo;
                if (GuiInfo.TryGetValue(player.userID, out guiInfo)) CuiHelper.DestroyUi(player, guiInfo);
            }
        }
        ...
        public void findTarget(BasePlayer player, HitInfo hitInfo)
        {
            ...
            foreach (BaseEntity foundEntity in plist)
            {
                ...
                if (config.FreezeArrowConfig.freezePlayers)
                {
                    ...
                    if (isPlayer && !FrozenEntityList.Contains(isPlayer.net.ID) && isPlayer != player)
                    {
                        ...
                        FrozenEntityList.Add(isPlayer.net.ID);
                        timer.Once(config.FreezeArrowConfig.ReFreezeCooldown, () => FrozenEntityList.Remove(player.net.ID));
                    }
                }
