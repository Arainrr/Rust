...
{
    [Info("WipeProtection", "FastBurst", "2.0.4", ResourceId = 2722)]
    ...
    class WipeProtection : RustPlugin
    {
            ...
            timer.Every(30f, () => {
                ...
                if (DateTime.Now >= Convert.ToDateTime(storedData.RaidStartTime))
                {
                    if (configData.Settings.broadcastend)
                        covalence.Server.Broadcast(msg("raidprotection_ended"));
        ...
        private void OnUserConnected(IPlayer player)
        {
            ...
            player.Message(msg("adminmsg"), "<color=orange>[WipeProtection]</color>", storedData.RaidStartTime, remaining);
        }
