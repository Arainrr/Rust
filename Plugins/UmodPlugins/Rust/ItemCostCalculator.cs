...
{
    [Info("Item Cost Calculator", "Absolut/Arainrr", "2.0.12", ResourceId = 2109)]
        ...
        private void CrateDataFile(Dictionary<ItemDefinition, double> costs, bool isGUIShop = true)
        {
            if (isGUIShop)
            {
                ...
                foreach (var entry in costs)
                {
                    ...
                    if (GUIShopData.items.ContainsKey(displayNameKey))
                    {
                    ...
                    if (!GUIShopData.items.ContainsKey(displayNameKey))
                    {
                        ...
                        GUIShopData.items.Add(displayNameKey, new ShopItem
                        {
                            DisplayName = displayName,
                            Shortname = entry.Key.shortname,
                            EnableBuy = true,
                            EnableSell = true,
                            Image = imageUrl ?? "https://rustlabs.com/img/items180/" + entry.Key.shortname + ".png",
