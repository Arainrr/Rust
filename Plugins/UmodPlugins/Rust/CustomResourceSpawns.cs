...
{
    [Info("Custom Resource Spawns", "k1lly0u", "0.2.28", ResourceId = 1783)]
    ...
    class CustomResourceSpawns : RustPlugin
    {
        ...
        private void Unload()
        {
            ...
            foreach (BaseEntity resource in resourceCache)
            {
                if (resource is OreResourceEntity)
                    (resource as OreResourceEntity).Kill();
                resource.Kill();
            }
            resourceCache.Clear();
        }
        ...
        private void chatResourceSpawn(BasePlayer player, string command, string[] args)
        {
            ...
            if (args.Length >= 1)
            {
                switch (args[0].ToLower())
                {
                    ...
                    case "remove":
                        {
                            if (args.Length >= 2 && args[1].ToLower() == "near")
                            {
                                ...
                                if (resources != null)
                                {
                                    ...
                                    {
                                        BaseEntity entity = FindResourcePos(entry);
                                        if (entity != null)
                                        {
                            ...
                            BaseEntity resource = FindResource(player);
                            if (resource != null)
                            {
                    ...
                    case "near":
                        {
                            ...
                            List<Vector3> resources = FindInRadius(player.transform.position, rad);
                            if (resources != null)
                            {
                    ...
                    case "wipe":
                        {
                            int count = crsData.resources.Count;
                            foreach (BaseEntity resource in resourceCache)
                            {
                                if (resource is OreResourceEntity)
                                    (resource as OreResourceEntity)._hotSpot.Kill();
                                resource.Kill();
                            }
