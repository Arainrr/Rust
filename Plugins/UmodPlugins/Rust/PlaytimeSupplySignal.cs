...
{
    [Info("Playtime Supply Signal", "Krungh Crow", "1.1.2")]
...
*=======================================================================================================================*/
    ...
    public class PlaytimeSupplySignal : RustPlugin
    {
        ...
        string VASpawn = "crate";                       // VehicleAirdrops signal to give on spawn
        int VASpawnAmount = 1;                          // Amount of VehicleAirdrops signals to give on spawn
        string VABonus = "crate";                       // VehicleAirdrops signal to give on bonus
    ...
    protected override void LoadDefaultConfig()
        ...
        private void LoadVariables()
        {
            ...
            VASpawn = Convert.ToString(GetConfig("VehicleAirdrops Spawn settings", "VehicleAirdrops signal when spawned", "crate"));
            VASpawnAmount = Convert.ToInt32(GetConfig("VehicleAirdrops Spawn settings", "VehicleAirdrops signal amount when spawned", "1"));
            VABonus = Convert.ToString(GetConfig("VehicleAirdrops Bonus settings", "VehicleAirdrops signal on bonus", "crate"));
            ...
            foreach (BasePlayer player in playerlist)
            {
                ...
                if (storedData.playerIDhadfirst.ContainsKey(player.userID) == false)
                {
                    if (spawnbonus == true)
                    {
                        ...
                        {
                                Server.Command($"signal_give {player.userID} {VABonus} {VASpawnAmount}");
                        }
                ...
                else
                {
                    ...
                    if (newtime >= bonusrate)
                    {
                        ...
                        else if (VehicleAirdrops == true && VAUse == true)
                        {
                            if (permission.UserHasPermission(player.UserIDString, Bonus_Perm))
                            {
                                Server.Command($"signal_give {player.userID} {VASpawn} {VABonusAmount}");
                                Player.Message(player, $"<color={ChatColor}> {lang.GetMessage("ItemMsg", this, player.UserIDString)}</color>", $"<color={PrefixColor}> {Prefix} </color>", SteamIDIcon);
                            }
                        }
                    }
                }
