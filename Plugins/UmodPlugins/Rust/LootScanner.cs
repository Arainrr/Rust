...
{
    [Info("Loot Scanner", "Sorrow", "0.3.2")]
    ...
    class LootScanner : RustPlugin
    {
        ...
        private void OnPlayerInput(BasePlayer player, InputState input)
        {
            ...
            if (!input.WasJustPressed(BUTTON.USE) || player.GetActiveItem()?.info.itemid != BinocularsId) return;
                ...
                entity.OwnerID == 0 && permission.UserHasPermission(player.UserIDString, PermissionWorld))
            {
                ...
                {
                    var scannerMessage = "";
                    if (entity.ShortPrefabName == SupplyDrop && HideAirdrop || entity.ShortPrefabName == CH47Crate && HideCh47Crate ||
                        LootContainer.spawnType.CRASHSITE.Equals(entity.GetComponent<LootContainer>()?.SpawnType) && HideCrashSiteCrate ||
