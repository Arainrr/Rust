...
{
    [Info("MapNote Teleport", "MON@H", "2.1.1")]
    ...
    public class MapNoteTeleport : CovalencePlugin
    {
        ...
        protected override void LoadDefaultMessages()
        {
            ...
            {
                ["CooldownEnded"] = "Godmode is now <color=#B22222>Disabled</color>. You can teleport again.",
                ...
                ["Teleported"] = "Teleported to <color=#FFA500>{0}</color>. Godmode is now <color=#228B22>Enabled</color> for <color=#FFA500>{1}</color>s while teleport is on Cooldown.",
            ...
            lang.RegisterMessages(new Dictionary<string, string>
            {
                ["CooldownEnded"] = "Режим Бога <color=#B22222>Откл.</color>. Можете телепортироваться снова.",
                ...
                ["Prefix"] = "<color=#00FFFF>[MapNote Teleport]</color>: ",
                ["Teleported"] = "Вы телепортированы на <color=#FFA500>{0}</color>. Режим Бога <color=#228B22>Вкл.</color> на <color=#FFA500>{1}</color> сек, пока телепорт перезаряжается.",
        ...
        private void TeleportToNote(BasePlayer player, MapNote note)
        {
            ...
            if (playerData.Notification)
                Print(player.IPlayer, Lang("Teleported", player.UserIDString, pos, playerData.Cooldown));
        ...
        {
            if (player == null || note == null || _playersWithGodMode.Contains(player.userID)) return;
        ...
        private object OnEntityTakeDamage(BasePlayer player, HitInfo info)
        {
            ...
            if (_playersWithGodMode.Contains(player.userID)) return true;
