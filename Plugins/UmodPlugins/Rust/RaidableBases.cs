...
{
    [Info("Raidable Bases", "nivex", "1.5.4")]
    ...
    class RaidableBases : RustPlugin
    {
        ...
        public class RaidableBase : FacepunchBehaviour
        {
            ...
            public Dictionary<string, BasePlayer> raiders;
            ...
            private void CreatePool()
            {
                ...
                raiders = Pool.Get<Dictionary<string, BasePlayer>>();
            ...
            private bool IsHogging(BasePlayer player)
            {
                ...
                {
                    if (!raid.IsPayLocked && raid.IsOpened && raid.BaseIndex != BaseIndex && raid.Any(player.userID, false, false))
                    {
            ...
            public void Despawn()
            {
                ...
                if (!IsUnloading)
                {
                    ServerMgr.Instance.StartCoroutine(Backbone.Plugin.UndoRoutine(BaseIndex, BuiltList.ToList()));
                }

                foreach (var raider in raiders)
                {
                    TrySetLockout(raider.Key);
                }
            ...
            public bool AddLooter(BasePlayer looter)
            {
                ...
                if (!raiders.ContainsKey(looter.UserIDString))
                {
                    raiders.Add(looter.UserIDString, looter);
                    return true;
                }
            ...
            public bool HasLockout(BasePlayer player)
            {
                ...
                if (!IsOpened && Any(player.userID, true, true))
                {
                    return false;
                }
            ...
            private void ClearEnemies()
            {
                ...
                foreach (string targetId in list)
                {
            ...
            private void SetContainers()
            {
                ...
                for (int i = 0; i < hits; i++)
                {
                    ...
                    if (!_containers.Contains(container) && (container is BoxStorage || container is BuildingPrivlidge))
                    {
            ...
            private IEnumerator EntitySetup()
            {
                ...
                var _instruction = ConVar.FPS.limit > 80 ? Coroutines.WaitForSeconds(INSTRUCTION_TIME) : null;
                ...
                Invoke(SetupLoot, 0.1f); // invokes will allow the code to continue if an exception is thrown
                Invoke(Subscribe, 0.2f);
                Invoke(CreateZoneWalls, 0.3f);
                Invoke(KillTrees, 0.4f);
                Invoke(CreateGenericMarker, 0.5f);
                Invoke(CreateSpheres, 0.6f);
                Invoke(EjectSleepers, 0.7f);
                Invoke(UpdateMarker, 0.8f);
                Invoke(TryStartPlayingWithFire, 0.9f);
                Invoke(SetupLights, 1f);
                Invoke(SetupDoorControllers, 1.1f);
                Invoke(SetupDoors, 1.2f);
                Invoke(SetupContainers, 1.3f);
                Invoke(MakeAnnouncements, 1.4f);
                Invoke(CheckDespawn, 1.5f);
                InvokeRepeating(Protector, 1.6f, 1f);
            ...
            {
                _containers.RemoveAll(x => x == null || x.IsDestroyed || x.transform == null);
                ...
                if (Buildings.BaseLoot.TryGetValue(BaseName, out lootList))
                {
                    TakeLootFrom(lootList);
                }
                ...
                if (_config.Settings.Management.IgnoreContainedLoot)
                {
                    ...
                    {
                        if (_containers[i].inventory.IsEmpty())
                        {
            ...
            private void SpawnLoot(StorageContainer container, List<TreasureItem> loot, int total)
            {
                ...
                {
                    if (loot.Count == 0 || container.inventory.IsFull())
                    {
                        break;
                    }
            ...
            public bool Any(ulong targetId, bool checkIntruders, bool checkFriends)
            {
            ...
            {
                if (player.IsNpc || (Type == RaidableType.None && !player.IsSleeping()) || JustDied(player))
                {
                    return;
                }
            ...
            private bool CanEject(BasePlayer target)
            {
                ...
                if (Backbone.HasPermission(target.UserIDString, banPermission))
                {
                    ...
                    {
                        Backbone.Message(target, "Banned");
                    }
        ...
        private void OnEntityBuilt(Planner planner, GameObject go)
        {
            ...
            if (flag && (e.prefabID == 3234260181 || e.prefabID == 72949757))
            {
        ...
        private void OnEntityDeath(IOEntity io, HitInfo hitInfo)
        {
            ...
            if (turret.IsValid() && !turret.IsDestroyed)
            {
        ...
        private void OnEntitySpawned(PlayerCorpse corpse)
        {
            ...
            if (corpse.playerSteamID.IsSteamId())
            {
                ...
                if (raid.Any(corpse.playerSteamID, false, true) || raid.owner.IsValid() && raid.IsAlly(player))
                {
        ...
        private object CanEntityTakeDamage(BaseEntity entity, HitInfo hitInfo)
        {
            ...
            if (attacker.userID == victim.userID || _config.TruePVE.ServerWidePVP)
            {
                return true;
            }
        ...
        private HandledResult HandlePlayerDamage(BasePlayer victim, HitInfo hitInfo)
        {
            ...
            else if (RaidableBase.Has(victim.userID))
            {
                ...
                if (entity.IsValid() && entity.OwnerID.IsSteamId() && entity is AutoTurret)
                {
                    return HandledResult.Allowed;
                }
        ...
        private Vector3 GetEventPosition(BuildingOptions options, BasePlayer owner, float distanceFrom, bool checkTerrain, RaidableSpawns rs, RaidableType type)
        {
            ...
            while (rs.Count > 0 && attempts > 0)
            {
                ...
                if (!options.Submerged && options.SubmergedAreaCheck && !raidSpawns.ContainsKey(type) && IsSubmerged(position, Mathf.Max(options.ArenaWalls.Radius, options.ProtectionRadius)))
                {
                    continue;
                }
        ...
        private bool HasActivePlayerBuildingPrivilege(Vector3 target, float radius)
        {
            bool flag = false;
        ...
        private bool IsAreaSafe(ref Vector3 position, float radius, int layers, RaidableType type = RaidableType.None, float awr = 0f)
        {
            if (awr > radius)
            {
                radius = awr + 2.5f;
        ...
        private IEnumerator UndoRoutine(int baseIndex, List<uint> builtList)
        {
            ...
            //bool isValidNetworkID = networkID != 0u;
            //bool isValidBuildingID = buildingID != 0u;
            var _instruction = ConVar.FPS.limit > 80 ? Coroutines.WaitForSeconds(INSTRUCTION_TIME) : null;
            ...
            while (entities.Count > 0)
            {
                ...
                /*if (e.net != null && !builtList.Contains(e.net.ID))
                {
                    if (isValidNetworkID && e.net.ID < networkID) // 1.5.3: this should never happen!
                    {
                        entities.Remove(e);
                        continue;
                    }

                    if (isValidBuildingID && e is DecayEntity && (e as DecayEntity).buildingID != buildingID) // 1.5.3: ^
                    {
                        entities.Remove(e);
                        continue;
                    }
        ...
        private void UndoPaste(Vector3 position, RaidableBase raid, int baseIndex, List<uint> builtList)
        {
            ...
            if (_config.Settings.Management.DespawnMinutes > 0)
            {
                ...
                timer.Once(time, () =>
                {
                    if (!IsUnloading)
                    {
                        ServerMgr.Instance.StartCoroutine(UndoRoutine(baseIndex, builtList));
                    }
                });
            }
            else ServerMgr.Instance.StartCoroutine(UndoRoutine(baseIndex, builtList));
        }
        ...
        private void CommandToggle(IPlayer p, string command, string[] args)
        {
            ...
            {
                p.Reply($"Toggled maintained events {((maintainedEnabled = !maintainedEnabled) ? "on" : "off")}");
            }

            if (_config.Settings.Schedule.Enabled)
            {
                p.Reply($"Toggled scheduled events {((scheduledEnabled = !scheduledEnabled) ? "on" : "off")}");
            }
        }
        ...
        private void CommandPopulate(IPlayer p, string command, string[] args)
        {
            ...
            string file = $"{Interface.Oxide.ConfigDirectory}{Path.DirectorySeparatorChar}{Name}.backup.{DateTime.Now:yyyy-MM-dd hh-mm-ss}.json";
        ...
        private void AddToList(LootType lootType, List<TreasureItem> source)
        {
            ...
            {
                if (!lootList.Contains(ti, itemComparer))
                {
                    lootList.Add(ti);
                }
            }

            string file = $"{Name}{Path.DirectorySeparatorChar}Default_Loot{Path.DirectorySeparatorChar}{lootType}";
        ...
        protected void LoadProfiles()
        {
            ...
            string empty = $"{folder}{Path.DirectorySeparatorChar}_empty_file";
            ...
            foreach (string file in files)
            {
                ...
                try
                {
                    if (file.EndsWith("_empty_file.json"))
                    {
                        continue;
                    }
        ...
        protected void LoadTables()
        {
            ...
            {
                Buildings.DifficultyLoot[lootType] = GetTable(lootType == LootType.Default ? $"{Name}{Path.DirectorySeparatorChar}Default_Loot" : $"{Name}{Path.DirectorySeparatorChar}Difficulty_Loot{Path.DirectorySeparatorChar}{lootType}");
            }

            foreach (DayOfWeek day in Enum.GetValues(typeof(DayOfWeek)))
            {
                Buildings.WeekdayLoot[day] = GetTable($"{Name}{Path.DirectorySeparatorChar}Weekday_Loot{Path.DirectorySeparatorChar}{day}");
            }
        }
        ...
        private void LoadBaseTables()
        {
            foreach (var entry in Buildings.Profiles)
            {
                ...
                var lootList = GetTable($"{Name}{Path.DirectorySeparatorChar}Base_Loot{Path.DirectorySeparatorChar}{entry.Key}");
        ...
        protected override void LoadConfig()
        {
            ...
            SaveConfig();
            Config.WriteObject(_config, false, $"{Interface.Oxide.ConfigDirectory}{Path.DirectorySeparatorChar}{Name}.backup.json");
        }
